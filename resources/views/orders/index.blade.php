                            ...
                            <tbody>
                                @forelse ($orders as $index => $order)
                                    <tr>
                                        ...
                                        <td class="border px-4 py-2 break-words max-w-xs">{{ $order->project_desc }}
                                        </td>
                                        ...
                                        <td class="border px-4 py-2">
                                            @if ($order->status === 'success')
                                                <span class="text-green-600 font-semibold">Transaksi sukses</span>
                                            @elseif ($order->status === 'failed')
                                                <span class="text-red-600 font-semibold">Transaksi ditolak</span>
                                            @else
                                                <div class="flex gap-2">
                                                    <form action="{{ route('orders.verify', $order->id) }}"
                                                        method="POST" class="inline-block"
                                                        onsubmit="return confirm('Verifikasi order ini?')">
                                                        @csrf
                                                        <button
                                                            class="font-bold bg-green-500 hover:bg-green-600 text-white px-3 py-1 rounded text-sm">
                                                            Verifikasi
                                                        </button>
                                                    </form>
                                                    ...
                                                    <form action="{{ route('orders.reject', $order->id) }}"
                                                        ...
                                                        @csrf
                                                        <button
                                                            class="font-bold bg-yellow-500 hover:bg-yellow-600 text-white px-3 py-1 rounded text-sm">
                                                            Tolak
                                                        </button>
                                                    </form>
                                                    ...
                                                    <form action="{{ route('orders.destroy', $order->id) }}"
                                                        ...
                                                        @method('DELETE')
                                                        <button
                                                            class="font-bold bg-red-500 hover:bg-red-600 text-white px-3 py-1 rounded text-sm">
                                                            Hapus
                                                        </button>
                                                    </form>
                                                </div>
